...
public class ImageController {
    ...
    //adding image id here as request mapping as well so our request to /images/id/title will be captured and this controller will be called
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session) {
        ...
        //check if current user id matches with image owner userid
        //if it matches user can edit image and redirected to images/edit
        //else request will be transferred to images/image with error message for user to display
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId, Model model, HttpSession session) {
        // get logged in user form session loggeduser
        User loggedInUser = (User) session.getAttribute("loggeduser");
        Image image = imageService.getImage(imageId);
        //check if current user id matches with image owner userid
        //if it matches user can edit image and redirected to image and delete image
        //else request will be transferred to images/image with error message for user to display
        if(image.getUser().getId()!=loggedInUser.getId()){
            String tags = convertTagsToString(image.getTags());
            model.addAttribute("image", image);
